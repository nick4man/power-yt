...
- Проверить с хоста: `showmount -e <NAS_IP>`.
...
- **`/data` пуст в контейнере** → NFS не смонтирован или Docker стартовал раньше. `systemctl status mnt-storage-data.mount`, проверь сеть до `<NAS_IP>` и экспорт (`showmount -e`).
